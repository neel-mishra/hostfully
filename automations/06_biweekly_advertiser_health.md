...
```bash
python3 automations/lib/meta_ads_api.py campaign-performance --id CAMPAIGN_ID --date-preset last_30d
```
...
```bash
python3 "python scripts/customer success agent/advertiser_health.py"
```
...
```bash
python3 "python scripts/customer success agent/churn_analyzer.py" --advertiser "ADVERTISER_NAME"
```
...
```bash
python3 automations/lib/gsheets_api.py create --title "Advertiser Health Tracker"
```

Append a row per advertiser:

```bash
python3 automations/lib/gsheets_api.py update --title "Advertiser Health Tracker" --range "Sheet1" --values '[["date","advertiser","risk_level","spend_30d","ctr","cpc","days_to_renewal","risk_factor","action"]]'
```

## Step 8: Push Report to Google Docs

```bash
python3 automations/lib/gdocs_api.py create --title "Advertiser Health Report - DATE"
python3 automations/lib/gdocs_api.py update --doc-name "Advertiser Health Report - DATE" --text "REPORT_CONTENT" --location start
```
