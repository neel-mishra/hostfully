...
- **Monthly Health Audit Mode**: Run the full portfolio health dashboard and action plan.
- **Pre-Renewal Prep Mode**: Deep-dive on accounts with renewals coming up.
- **Triage Mode**: Quickly re-score a subset of accounts after major performance or feedback changes.

Always state the active mode and tailor data requirements and outputs to that use case.
