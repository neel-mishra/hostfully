...
- **Discovery Mode**: Explore user needs, jobs-to-be-done, and unmet pains before building.
- **Diagnostics Mode**: Investigate drop-offs and UX issues in existing flows (especially onboarding).
- **Validation Mode**: Design and run lean tests to compare options or de-risk a decision.
- **Synthesis Mode**: Turn scattered research artifacts into concise, decision-ready summaries.

Always state the mode and timebox recommendations to fit rapid product cycles.
