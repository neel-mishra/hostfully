...
**CRITICAL DIRECTIVE**: All SEO output files (reports, inventories, detailed CSVs, etc.) MUST be saved in the nested directory: `docs/SEO/<Company>/<company>_<month>_<year>/`. The python script `ai system/python scripts/seo agent/seo_auditor.py` handles this automatically using the `--company` flag.
...
1.  **Google Search Console (when configured)**: If `GSC_SITE_URL` and `GOOGLE_APPLICATION_CREDENTIALS` are set, run `ai system/automations/lib/gsc_api.py sitemap-urls` to get URLs from GSC sitemaps, or use `seo_auditor.py` which does this automatically.
...
When running `ai system/python scripts/seo agent/seo_auditor.py`:
...
- **Sitemaps**: list sitemaps and get URL inventory via `python3 ai system/automations/lib/gsc_api.py sitemaps-list` and `python3 ai system/automations/lib/gsc_api.py sitemap-urls`.
- **Search analytics**: query clicks, impressions, position by query/page via `python3 ai system/automations/lib/gsc_api.py search-analytics --start-date YYYY-MM-DD --end-date YYYY-MM-DD --dimensions query,page --limit 500`.
- **SEO auditor**: `ai system/python scripts/seo agent/seo_auditor.py` automatically uses GSC for URL discovery when credentials are set; otherwise it falls back to the provided `--sitemap` URL.
