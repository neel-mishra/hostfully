...
def main():
    ...
    idempotency_key = args.idempotency_key or logical_period_idempotency_key(
        ...
        suffix=f"{args.command}:{args.date_preset}:{args.id or args.campaign_id or args.ad_set_id or ''}:{args.since or ''}:{args.until or ''}",
    )
    if (not args.allow_duplicate_run) and is_duplicate_success(idempotency_key):
        ...
        emit_run_event("meta_ads_api", "skipped", step=args.command, idempotency_key=idempotency_key)
        return

    missing = preflight_env(["META_ACCESS_TOKEN", "META_AD_ACCOUNT_ID"])
    if missing:
        print(json.dumps({"error": "missing_env", "missing": missing}))
        emit_run_event("meta_ads_api", "failed", step=args.command, idempotency_key=idempotency_key, details={"missing_env": missing})
        sys.exit(1)

    emit_run_event("meta_ads_api", "started", step=args.command, idempotency_key=idempotency_key)
    ...
    emit_run_event("meta_ads_api", "success", step=args.command, idempotency_key=idempotency_key)
