...
def main():
    ...
    idempotency_key = args.idempotency_key or logical_period_idempotency_key(
        ...
        suffix=f"{args.command}:{args.title or args.doc_id or args.doc_name or ''}",
    )
    if (not args.allow_duplicate_run) and is_duplicate_success(idempotency_key):
        ...
        emit_run_event("gdocs_api", "skipped", step=args.command, idempotency_key=idempotency_key)
        return

    missing = preflight_env(["GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "GOOGLE_REFRESH_TOKEN"])
    if missing:
        print(json.dumps({"error": "missing_env", "missing": missing}))
        emit_run_event("gdocs_api", "failed", step=args.command, idempotency_key=idempotency_key, details={"missing_env": missing})
        sys.exit(1)

    emit_run_event("gdocs_api", "started", step=args.command, idempotency_key=idempotency_key)
    ...
    emit_run_event("gdocs_api", "success", step=args.command, idempotency_key=idempotency_key)
