...
| **agents/gtm team/marketing/seo-audit-agent.md** | Mentions “Search Console coverage report” and “Core Web Vitals report”. | **Make API explicit**: add a bullet that the agent (or the human running it) can use the Search Console API for “view properties/sitemaps” and “test individual pages” (URL Inspection) when GSC access is available. Link: [Search Console API – About](https://developers.google.com/webmaster-tools/about). |
| **agents/seo-aeo/seo-audit-agent.md** | Same as above. | Same as above — reference the API for coverage and URL inspection. |
