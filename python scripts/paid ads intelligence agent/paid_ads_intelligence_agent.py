...
def pull_full_meta_data(since: str, until: str):
    ...
    for camp in campaigns:
        ...
        # If the API returns an empty data list for the given time_range, we set {} so downstream
        # logic treats spend/impressions/actions as 0 for this period.
        if isinstance(camp_insights, dict) and "data" in camp_insights:
            ci_data = camp_insights.get("data") or []
            camp["_campaign_insights"] = ci_data[0] if ci_data else {}
        else:
...
def gaql_query(query):
    """Execute GAQL query. On 404, tries Search API fallback and different login-customer-id."""
    ...
    # 404: diagnose and retry with listAccessibleCustomers + alternate configs
    ...
    if accessible is not None:
        ...
        if GADS_CUSTOMER_ID not in ids:
            for lid in ids:
                if lid == GADS_CUSTOMER_ID:
                    continue
                rows, err2 = _gaql_request(query, use_search_stream=True, login_customer_id=lid)
                if not err2:
                    print(f"   ✅ Resolved 404 using login-customer-id={lid} (add GOOGLE_ADS_MANAGER_ID={lid} to .env)")
                    return rows
                rows, err2 = _gaql_request(query, use_search_stream=False, login_customer_id=lid)
                if not err2:
                    print(f"   ✅ Resolved 404 via Search API with login-customer-id={lid}")
                    return rows
        elif GADS_MANAGER_ID:
            rows, err2 = _gaql_request(query, use_search_stream=True, omit_login=True)
            if not err2:
                print(f"   ✅ Resolved 404 by omitting login-customer-id")
                return rows

    # Fallback: non-streaming Search API
    rows, err2 = _gaql_request(query, use_search_stream=False)
    if not err2:
        return rows

    hint = " (Add GOOGLE_ADS_MANAGER_ID to .env if account is under MCC. See accessible accounts above.)"
    return [{"error": f"{err}{hint}", "body": ""}]
